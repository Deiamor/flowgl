...
export class HandleProgram {
  ...
  private circleBuffer:  DynamicBuffer
  private uCircleMatrix: WebGLUniformLocation

  private edgeProgram: WebGLProgram
  private edgeVao:     WebGLVertexArrayObject
  private edgeBuffer:  DynamicBuffer
  private uEdgeMatrix: WebGLUniformLocation
  ...
  constructor(gl: WebGL2RenderingContext) {
    ...
    if (!quadBuf) throw new Error('HandleProgram: createBuffer failed')
    ...
    if (!circleVao) throw new Error('HandleProgram: createVertexArray failed')
    ...
    const defC = (name: string, size: number, offset: number): void => {
    ...
    if (!cMat) throw new Error('HandleProgram: u_matrix not found (circle)')
    ...
    if (!edgeVao) throw new Error('HandleProgram: createVertexArray failed (edge)')
    this.edgeVao = edgeVao
    gl.bindVertexArray(this.edgeVao)
    gl.bindBuffer(gl.ARRAY_BUFFER, this.edgeBuffer.buffer)

    const eStride = EDGE_FLOATS_PER_VERT * 4
    const defE = (name: string, size: number, offset: number): void => {
    ...
    if (!eMat || !dLen || !gLen) throw new Error('HandleProgram: edge uniforms not found')
  ...
  render(
    ...
    if (rerouteState) this.renderRerouteEdge(rerouteState, matrix, zoom)
  }
  ...
  private renderHandles(
    ...
    const showForIds = new Set<string>()
    if (s.hoveredNodeId)          showForIds.add(s.hoveredNodeId)
    if (s.connectingFrom?.nodeId) showForIds.add(s.connectingFrom.nodeId)
    if (s.targetNodeId)           showForIds.add(s.targetNodeId)   // target node during drag
    if (showForIds.size === 0) return
    ...
    for (const nodeId of showForIds) {
      ...
      for (const h of getHandlePositions(node)) {
        const isHovered =
          s.hoveredHandle?.nodeId === h.nodeId &&
  ...
  private renderPendingEdge(
    ...
    const [c1x, c1y, c2x, c2y] = edgeControlPoints(
      sx, sy, s.connectingFrom.side,
      ex, ey, s.targetHandle ?? undefined,
    )
    const hw = (2 / zoom) / 2
    const data = buildBezierStrip(
  ...
  private renderRerouteEdge(
    ...
    let sx: number, sy: number, srcSide: string | undefined
    let ex: number, ey: number, tgtSide: string | undefined

    if (s.movingEnd === 'target') {
      sx = s.fixedHandle.wx;  sy = s.fixedHandle.wy;  srcSide = s.fixedHandle.side
      ex = s.pendingEndWx;    ey = s.pendingEndWy;    tgtSide = s.targetHandle ?? undefined
    } else {
      sx = s.pendingEndWx;    sy = s.pendingEndWy;    srcSide = s.targetHandle ?? undefined
      ex = s.fixedHandle.wx;  ey = s.fixedHandle.wy;  tgtSide = s.fixedHandle.side
    }
